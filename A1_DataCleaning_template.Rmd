...
```{r}
# Changing the column name from Child.ID to SUBJ on the demo train data set
names(demo_train)[names(demo_train) == "Child.ID"] <- "SUBJ"
# Doing the same for the visit column
...
```{r}
# Changing the visits to exclude "visit" and ".".
...
```{r}
# Removing all "." from the subject names
demo_train$SUBJ <- str_remove_all(demo_train$SUBJ, "\\.")
LU_train$SUBJ <- str_remove_all(LU_train$SUBJ, "\\.")
token_train$SUBJ <- str_remove_all(token_train$SUBJ, "\\.")
...
```{r}
# Selecting the relevant columns before the merging of data frames.
demo_train_subset <- select(demo_train, SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

LU_train_subset <- select(LU_train, SUBJ, VISIT, MOT_MLU, CHI_MLU)

token_train_subset <- select(token_train, SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
```{r}
# Merging the data sets in two steps
train2 <- merge(demo_train_subset, LU_train_subset)
train <- merge(token_train_subset, train2)
...
```{r}
# Making the new data frame for visit 1
visit1 <- train[train$VISIT == "1",]

# Selecting the relevant columns
visit1 <- select(visit1, SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
...
# Merging the new data frame with the old one
mergedTrain <- merge(visit1, train)
...
```{r}
# Anonymizing the participants by giving them numbers instead of names
mergedTrain$SUBJ <- as.factor(mergedTrain$SUBJ)

# Making a list with numbers for all the participants
a <- seq(1, 61)

# Assigning the list to the subjects
levels(mergedTrain$SUBJ) <- a


# Changing the variables for gender to F and M
mergedTrain$Gender[mergedTrain$Gender == "1"] <- "F"
mergedTrain$Gender[mergedTrain$Gender == "2"] <- "M"

# Changing the variables for diagnosis to ASD and TD
mergedTrain$Diagnosis <- as.character(mergedTrain$Diagnosis)
mergedTrain$Diagnosis[mergedTrain$Diagnosis == "A"] <- "ASD"
mergedTrain$Diagnosis[mergedTrain$Diagnosis == "B"] <- "TD"

write.csv(mergedTrain, file = "train.csv")
...
```{r}
# 1. Filtering out the kids with a mean length utterance of above 2.7 across visits
utt_over_2.7 <- filter(mergedTrain, CHI_MLU > 2.7) %>%
  group_by(., SUBJ) %>% # Grouping by subject
  summarise(., n()) # Summarizing the subjects with a mlu above 2.7 and the amount of visits with mlu > 2.7

# 2. Filtering out the kids with a mean length utterance below 1.5 at visit 1
utt_under_1.5 <- filter(mergedTrain, CHI_MLU < 1.5 & VISIT == 1)

# 3. Filtering out the kids with less visits than 6.
not_complete <- group_by(mergedTrain, SUBJ) %>% # Grouping by subject
  summarise(., "number" = n()) %>% # Summarizing the number of observations
  filter(., number < 6) # Filtering out the ones with less visits than 6
```
...
```{r}
# Filter by visit 6
sixth_visit <- filter(mergedTrain, VISIT == 6) %>%
  arrange(., desc(tokens_CHI)) # Arrange by descending amount of words, starting with the subject who produced most words.
# Kid number 55 produced most words

# Filter by visit 1
first_visit <- filter(mergedTrain, VISIT == 1) %>%
  arrange(., tokens_CHI)
# Kid number 57 produced least words
...
```{r}
# Selecting subject, diagnosis, mlu and word token for ASD subjects
small_train <- select(mergedTrain, SUBJ, Diagnosis, CHI_MLU, tokens_CHI) %>%
  filter(., Diagnosis == "ASD")

# If you include the name of a variable multiple times it still only appears once
...
```{r}
# 1. Mean of words produced across participants
mean_all <- mean(mergedTrain$tokens_CHI) 
# Merging into dataframe
mergedTrain$mean_all <- mean_all

# 2. Grouping by subject and summarizing the mean of words across visits
mean_chi <- group_by(mergedTrain, SUBJ) %>%
  summarise(., mean(tokens_CHI))
# Merging into dataframe
mergedTrain <- merge(mergedTrain, mean_chi)

# 3. The averaging doesn't take into account the diagnosis of the child. It is also interesting to look at the development of the child which is not possible solely from the averaging.
